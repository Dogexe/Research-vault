...
1. Preset provenance is real (named, stored, procedure-mapped values exist on real devices) but general and unvalidated against either delivered power or histologic outcome — so testing an actual manufacturer-labeled preset, rather than an arbitrary researcher-chosen bench power level, is a meaningful and currently untested experimental choice (see [[01 Projects/Diode Laser Biopsy]] §6, "Current evidence gap / novelty" — preset testing may be included only if a named manufacturer/device preset is directly tested).
