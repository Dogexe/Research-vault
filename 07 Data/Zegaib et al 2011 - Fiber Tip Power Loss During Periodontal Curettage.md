...
- INTERPRETATION (the source's own framing, not independently re-derived here): the small pre-contact fiber-transmission loss (9%) versus the much larger progressive loss with repeated contact applications (up to 40.26%) is attributed by the authors to tip fouling/debris from gingival tissue contact during curettage, not to intrinsic fiber transmission loss — a within-session [[03 Concepts/Glossary/Output Stability|output-stability]] decline, distinct from a single-point set-vs-measured [[03 Concepts/Glossary/Power Loss|power-loss]] discrepancy.
