...
Actual tissue exposure is not fully described by a nominal console setting. Relevant modifiers include wavelength, set power, independently measured delivered power, CW/PW and pulse parameters, fiber diameter/spot size, initiated vs. non-initiated tip, contact mode, incision speed/movement, exposure duration, passes, cooling, and tissue properties. No missing parameter is inferred; absent values remain `UNKNOWN`.

The project does not ask which setting is “best,” establish clinical diagnostic success, prove clinical harm, or attribute misuse, negligence, or inadequate training to an operator. Ex vivo findings require separate clinical validation before they can support clinical diagnostic claims.
